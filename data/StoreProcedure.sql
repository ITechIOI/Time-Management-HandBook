...
BEGIN
	...
	AND IS_DELETED = 0 
	AND CAST(@TIME_NOW AS DATE) = CAST(_END_TIME AS DATE)
	AND CAST(@TIME_NOW AS DATE) = CAST(_START_TIME AS DATE)
END
...
@EMAIL VARCHAR(50), @NAME NVARCHAR(100), 
@END VARCHAR(20)
AS
BEGIN
	DECLARE @USER_ID VARCHAR(30), @EVENT_ID VARCHAR(30)
...
@EVENT_ID VARCHAR(30),
@SUMMARY NVARCHAR(100), @LOCATION NVARCHAR(100),
@CREATING_TIME VARCHAR(40), @END_TIME VARCHAR(40),
@NOTIFICATION VARCHAR(100), @DESCRIPTION NVARCHAR(200),
@FINISH_TIME VARCHAR(100),
@COLOR INT
AS
BEGIN
	DECLARE @CREATE DATETIME, @END DATETIME, @FINISH DATETIME
	SET @CREATE = CAST(@CREATING_TIME AS DATETIME)
	SET @END = CAST(@END_TIME AS DATETIME)

	IF (@FINISH IS NULL)
	BEGIN
		UPDATE TASK SET NAME = @SUMMARY,
		LOCATION = @LOCATION, CREATING_TIME = @CREATE,
		END_TIME = @END, NOTIFICATION_PERIOD = @NOTIFICATION,
		DESCRIPTION = @DESCRIPTION, FINISHED_TIME = NULL,
	...
	BEGIN
		...
		LOCATION = @LOCATION, CREATING_TIME = @CREATE,
		END_TIME = @END, NOTIFICATION_PERIOD = @NOTIFICATION,
		DESCRIPTION = @DESCRIPTION, FINISHED_TIME = @FINISH,
		COLOR = @COLOR WHERE TASK_ID = @EVENT_ID
	END
END
...
CREATE OR ALTER PROC USP_GET_EVENT_OF_THE_DAY_BY_DAY_OF_MONTH
@EMAIL VARCHAR(50), @DATETIME VARCHAR(25)
AS
BEGIN
	DECLARE @USERID VARCHAR(30), @TIME_NOW DATE
	...
	SELECT * FROM EVENT_OF_THE_DAY WHERE USERID = @USERID
	AND IS_DELETED = 0 
	AND MONTH(@TIME_NOW) = MONTH(CAST(_START_TIME AS DATE))
END

GO

CREATE OR ALTER PROC USP_GET_PROLONGED_EVENT_BY_DAY_OF_MONTH
@EMAIL VARCHAR(50), @DATETIME VARCHAR(25)
AS
BEGIN
	DECLARE @ID_USER VARCHAR(30),  @DATE_NOW DATE
	...
	AND IS_DELETED = 0 AND @DATE_NOW >= _START_DATE AND 
	@DATE_NOW <= _END_DATE AND IS_DELETED = 0
	AND MONTH(@DATE_NOW) <= MONTH(_END_DATE)
	AND MONTH(@DATE_NOW) >= MONTH(_START_DATE)
END

GO

CREATE OR ALTER PROC INSERT_RATING
@EMAIL VARCHAR(50), @RATE INT
AS
BEGIN
	DECLARE @ID_USER VARCHAR(30),  @DATE_NOW DATE
	SELECT @ID_USER = USERID FROM _USER 
